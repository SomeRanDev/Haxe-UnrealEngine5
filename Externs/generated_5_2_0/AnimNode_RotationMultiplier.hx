// Generated by Haxe-UnrealEngine5 (https://github.com/RobertBorghese/Haxe-UnrealEngine5)
package ue;

@:native("FAnimNode_RotationMultiplier")
@:include("BoneControllers/AnimNode_RotationMultiplier.h")
@:valueType
extern class AnimNode_RotationMultiplier extends AnimNode_SkeletalControlBase {
	public var TargetBone: BoneReference;
	public var SourceBone: BoneReference;
	public var Multiplier: ucpp.num.Float32;
	public var RotationAxisToRefer: TEnumAsByte<EBoneAxis>;
	public var bIsAdditive: Bool;

	@:native("FAnimNode_RotationMultiplier") public function new();
	@:native("FAnimNode_RotationMultiplier") public static function make(TargetBone: BoneReference, SourceBone: BoneReference, Multiplier: ucpp.num.Float32, RotationAxisToRefer: TEnumAsByte<EBoneAxis>, bIsAdditive: Bool): AnimNode_RotationMultiplier ;
}